      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex flex-wrap gap-2 justify-center mb-10">
          {(["All", "Cakes", "Food", "Interior", "Celebrations"] as Cat[]).map((c) => (
            <button
              key={c}
              onClick={() => setFilter(c)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition ${filter === c ? "bg-chocolate text-cream" : "bg-cream/70 text-chocolate hover:bg-peach/40"}`}
            >
              {c}
            </button>
          ))}
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 [&>*]:mb-4">
          {filtered.map((i, idx) => (
            <div
              key={idx}
              className="break-inside-avoid rounded-2xl overflow-hidden shadow-[var(--shadow-soft)] group"
            >
              <img
                src={i.src}
                alt={i.cat}
